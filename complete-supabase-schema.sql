-- Complete Supabase Database Schema for CurriJobs
-- Run this in your Supabase SQL Editor to add all missing tables and columns
...
-- 3. Create task_cancellations table if it doesn't exist
...
-- 5. Create offers table if it doesn't exist
...
-- 6. Create payments table if it doesn't exist
...
-- 7. Create reviews table if it doesn't exist
...
CREATE INDEX IF NOT EXISTS idx_task_cancellations_task_id ON task_cancellations(task_id);
...
-- 13. Create RLS policies for offers
...
-- 15. Create RLS policies for task_cancellations
DROP POLICY IF EXISTS "Task cancellations are viewable by task owner and canceller" ON task_cancellations;
CREATE POLICY "Task cancellations are viewable by task owner and canceller" ON task_cancellations
  FOR SELECT USING (
    auth.uid() = cancelled_by OR 
    auth.uid() IN (SELECT user_id FROM tasks WHERE id = task_id)
  );

DROP POLICY IF EXISTS "Users can insert their own cancellations" ON task_cancellations;
CREATE POLICY "Users can insert their own cancellations" ON task_cancellations
  FOR INSERT WITH CHECK (auth.uid() = cancelled_by);

-- 16. Create RLS policies for user_progress
...
-- 18. Create RLS policies for reviews
...
-- 19. Create function to handle user creation
...
-- 21. Create function to update updated_at timestamp
...
-- 24. Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Success message
SELECT 'Complete database schema updated successfully!' as status;
