...
export default function CreateTaskForm({ initialCoords = null, initialCategory = 'cleaning', onCancel, onSuccess }: CreateTaskFormProps) {
  ...
  const [coords, setCoords] = useState<Coordinates | null>(initialCoords);
        ...
        <View style={[styles.formContainer, { backgroundColor: '#FFFFFF' }]}>
          ...
          <View style={styles.categoryGrid}>
            {[
              ...
              { key: 'electrician', label: '⚡ Electricidad' },
              ...
              <TouchableOpacity
                key={cat.key}
                style={[styles.categoryPill, category === (cat.key as string) && { backgroundColor: '#5B21B6', borderColor: '#5B21B6' }]}
                onPress={() => setCategory(cat.key)}
                activeOpacity={0.8}
              >
                <Text style={[styles.categoryPillText, { color: category === cat.key ? '#FFFFFF' : '#111827' }]}>
                  {cat.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.inputGroup}>
            <TouchableOpacity
              ...
              onPress={() => { setPickerStep('date'); setPickerVisible(true); }}
              activeOpacity={0.8}
            >
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.mapPreviewContainer}>
              <MapView
                provider={isAmazonAndroid() ? undefined : (Platform.OS === 'ios' ? PROVIDER_GOOGLE : undefined)}
          ...
          <View style={{ flexDirection: 'row', gap: 12 }}>
            {onCancel && (
              <TouchableOpacity
                style={[styles.createButton, { backgroundColor: '#6B7280', flex: 1 }]}
                onPress={onCancel}
                disabled={loading}
                activeOpacity={0.8}
              >
            ...
            <TouchableOpacity
              style={[styles.createButton, { backgroundColor: '#FF6B35', flex: 1 }]}
